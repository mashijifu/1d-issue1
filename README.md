...
- Node.js **20.x** 推奨（Vercel / `.nvmrc` / `package.json` の `engines` / CI と一致）
- npm 10 以上
...
## Vercel で公開する（Hobby・トークンなし）

Next.js は Vercel が公式にホストしており、**追加の設定ファイルなしで**デプロイできます。ポートフォリオ用の**公開デモは `GITHUB_TOKEN` を設定しない**運用で問題ありません（未認証の GitHub API のレート制限内で動作します）。

1. このリポジトリを GitHub に push する（まだなら [GitHub で New repository](https://github.com/new) を作成し、remote を追加）。
2. [Vercel](https://vercel.com/) にログインし、「Add New…」→「Project」から該当 Git リポジトリをインポートする。
3. **Framework Preset** は `Next.js` のまま、**Build Command** は `npm run build`（既定）、**Install Command** は `npm install` または `npm ci`（既定）でよいです。Node.js バージョンは **20.x** を推奨（Project Settings → General → Node.js Version）。
4. **Environment Variables** には `GITHUB_TOKEN` を**入れない**（公開デモのままデプロイでよい）。必要なら `GITHUB_API_RATE_LIMIT_*` だけ設定可能。
5. 「Deploy」を実行する。完了後に発行された `*.vercel.app` の URL が公開 URL です。
...
利用規約・商用可否は [Vercel のプラン](https://vercel.com/pricing) を確認してください（Hobby は個人利用向けの条件があります）。
